...
log "Pulling latest published $RESULTS_DIR_REL snapshot from DVC"
uv run dvc pull "$DVC_FILE_REL"

mkdir -p "$RESULTS_DIR_REL"
log "Merging backed-up local files over pulled snapshot (local files win on overlap)"
rsync -a "$BACKUP_DIR/local_results/" "$RESULTS_DIR_REL/"

log "Re-staging merged $RESULTS_DIR_REL with DVC"
